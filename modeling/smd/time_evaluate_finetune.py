...
def get_training_file_for_KB(args,num_template,indx,tokenizer):
    train_kb = generate_dataset_FINETUNE(f"data/SMD/test-{num_template}/dialog_{indx}.txt",tokenizer)
...
if __name__ == "__main__":
    ...
    time_list = []
    templates = [10, 25, 50, 75, 100]

    for num_template in templates:
        print("num_template:", num_template)
        ...
        for i, conv in tqdm(enumerate(test),total=len(test)):
            if(len(conv["edges"]) > 0):
                ## load KB
                train_KB_loader = get_training_file_for_KB(args,num_template,i,tokenizer)
                args.n_epochs = 1
        ...
        time_list.append({"domain": conv["domain"], "num_template": num_template, "avg in sec": avg_time, "total time in sec": sum_time})

print(tabulate(time_list,headers="keys",tablefmt='latex',floatfmt=".3f",numalign="center"))