...
class Attention(nn.Module):
    ...
    def _attn(self, q, k, v, attention_mask=None, head_mask=None):
        ...
        if attention_mask is not None:
            b = attention_mask
        #TODO check why we need 1e4 * (1-b)
        w = w * b - 1e4 * (1 - b)
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    Examples::
        ...
        if nodes is not None and len(nodes.size())==3:
            hidden_states = hidden_states[:,node_size[1]:,:]
        # print(output_shape)
...
class DIALOGGPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        super(DIALOGGPT2LMHeadModel, self).__init__(config)
        self.transformer = GPT2Model(config)
